...
# envs/dev/main.tf
#
# Composes the modules for the DEV environment. Provider assumes the deploy
...
module "foundation" {
  ...
  # DEV: lighter durability settings (save cost; prod will enable these).
...
module "ingestion" {
  ...
  # Schedule OFF in DEV — run ingestion manually until validated.
...
module "search" {
  ...
  # dev settings (cost-conscious; prod will override)
  provisioned_concurrency = 0       # off in DEV (turn on for load tests / prod)
  waf_enabled             = true     # WAF on from the start
  cors_allowed_origins    = ["*"]    # DEV only — prod MUST scope to equinix.com
}
