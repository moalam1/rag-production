...
# envs/prod/main.tf
#
# Composes the modules for the PROD environment. Provider assumes the deploy
...
module "foundation" {
  ...
  # PROD: lighter durability settings (save cost; prod will enable these).
...
module "ingestion" {
  ...
  # Schedule OFF in PROD — run ingestion manually until validated.
...
module "search" {
  ...
  # PROD settings (cost-conscious; prod will override)
  provisioned_concurrency = 0       # off in PROD (turn on for load tests / prod)
  waf_enabled             = true     # WAF on from the start
  cors_allowed_origins    = ["*"]    # PROD only — prod MUST scope to equinix.com
}
