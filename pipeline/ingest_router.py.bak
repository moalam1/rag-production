...
def route_and_ingest(page: ParsedPage) -> list[str]:
    """
    ...
    if is_unchanged_by_timestamp(page.url, page.og_updated_time):
        logs.append("⏭️  og:updated_time unchanged — skipping.")
        return logs

    # ── Hash check: skip if content unchanged ─────────────────────────────────
    if is_unchanged(filename=page.document_family, content_hash=page.content_hash, url=page.url):
...
def _ingest_text(
    ...
    # Summary chunks — PDF and transcript only, not page teasers
    # Page teasers are already short summaries — no value adding them to summary index
...
def _ingest_pdf(
    ...
    # Write to temp dir and call ingester
